...
class quad_tree{
    ...
    void refresh(){
        ...
        if((nw->nw == NULL && ne->nw == NULL) && (se->nw == NULL && sw->nw == NULL)){
            if((nw->data == ne->data) && (sw->data == se->data) && (sw->data == nw->data)){
                data = nw->data;
                deleteChild();
                nw = ne = sw = se = NULL;
            }
    ...
    quad_tree* extractUtil(quad_tree &r, long long _x, long long _y, long long m, long long w){
        ...
        long long Xc = x+width/2;
        long long Yc = y+width/2;
...
public:
    ...
    void extract(long long _x, long long _y, long long m){
        ...
        deleteChild();
        if(t->nw != NULL){
            nw = new quad_tree(*t->nw); 
            ne = new quad_tree(*t->ne); 
            sw = new quad_tree(*t->sw); 
            se = new quad_tree(*t->se);
        }else{
            nw = ne = sw = se = NULL;
        }
        delete t;
